...
public class Drag : MonoBehaviour, IPointerDownHandler, IPointerUpHandler
{
        ...
        if(dragmode.isDragMode==true)       // 드래그 모드가 켜져야 움직임, 꺼지면 그림을 그리는 모드가 되는 것
        {
           ...
           MinusPosition = SqObject.transform.position - mousePosition1;
            ...
            if (SqObject.name == "Sq10")
            {
                ...
                if (SqObject.transform.position == Sq10Spawn)   // 마우스로 잡은 사물 좌표가 특정 위치라면 복사 함수를 실행
                {
                    Sq10spawn();
                }
            }
            ...
            if (SqObject.name == "Sq1")
            {
                ...
                if (SqObject.transform.position == Sq1Spawn)   // 마우스로 잡은 사물 좌표가 특정 위치라면 복사 함수를 실행
                {
                    Sq1spawn();
                }
            }
    ...
    */
    ...
    {
        Vector3 Sq10Spawn = new Vector3(-4.887f, 1.601f, 0);            // 해당 위치에서 복사됨
    ...
    {
        Vector3 Sq1Spawn = new Vector3(-3.988f, 1.672f, 0);              // 해당 위치에서 복사됨
